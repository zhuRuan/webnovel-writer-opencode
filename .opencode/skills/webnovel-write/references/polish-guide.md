...
- 禁止“首先/其次/最后”三段式说明写法（severity: `high`）。
- 禁止连续三句“主语 + 谓语 + 宾语”同构句（severity: `high`）。
- 禁止“他认为自己需要做三件事：一、二、三”清单化叙事（severity: `high`）。
- 禁止同一信息用不同表述重复 2-3 次（severity: `medium`）。
- 每 3-5 段至少出现一次句长变化（短句打断/插入动作/对话反问）。
- 段尾禁止机械总结句式（“这就是……的原因”“由此可见……”）（severity: `high`）。
...
- 禁止“说明书式对话”（完整解释背景、逻辑过满）（severity: `high`）。
- 对话必须有意图（试探/回避/施压/诱导/防御）。
- 允许真实口语停顿、打断、反问，避免全员“标准书面语”。
- 对话结束后禁止追加解释句（“他这么说是因为……”）（severity: `medium`）。
- 所有角色使用同一套反应模板（全员“瞳孔微缩”“心头一震”）→ severity: `high`。
...
- 禁止无过渡的情绪跳变（前一句愤怒、后一句平静）（severity: `high`）。
- 情绪转换必须通过行为/生理暗示过渡，禁止直接标签（“他感到X”）。
