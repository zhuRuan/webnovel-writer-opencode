...
若本次规划会直接落到具体章节，必须先刷新 Story System runtime 合同：
...
规划开始/结束都运行 `placeholder-scan`；发现占位先补齐，写章前不得保留 `[待...]` / `{占位}`。
计划完成后只向 `大纲/总纲.md` 渐进追加下一卷概要，不预填 V2-V20 空表。写回来自显式结构化文件 `大纲/第{volume_id}卷-总纲写回.json`。
...
| Step 6 | always | `../../references/genre-profiles.md`, `../../references/shared/strand-weave-pattern.md` |
| 章纲拆分 | always | `../../references/outlining/plot-signal-vs-spoiler.md` |
| Step 6 | 需要爽点/冲突 | `../../references/shared/cool-points-guide.md`, `references/outlining/conflict-design.md` |
| Step 6/7 | 特定题材节奏 | `references/outlining/genre-volume-pacing.md` |
| Step 7 | 需要追读力/章纲细化 | `../../references/reading-power-taxonomy.md`, `references/outlining/chapter-planning.md` |
| 卷级规划 | always | `reference_search.py --skill plan --table 场景写法 --query "卷级结构 叙事功能"` |
| 章纲拆分 | 新增角色出现 | `reference_search.py --skill plan --table 命名规则 --query "角色命名" --genre {题材}` |
...
执行前加载模板：`cat "${SKILL_ROOT}/../../templates/output/大纲-卷节拍表.md"`
...
执行前加载模板：`cat "${SKILL_ROOT}/../../templates/output/大纲-卷时间线.md"`
...
必须加载：`cat "${SKILL_ROOT}/../../references/genre-profiles.md" "${SKILL_ROOT}/../../references/shared/strand-weave-pattern.md"`

按需加载：`cat "${SKILL_ROOT}/../../references/shared/cool-points-guide.md" "${SKILL_ROOT}/references/outlining/conflict-design.md" "${SKILL_ROOT}/references/outlining/genre-volume-pacing.md" "$PROJECT_ROOT/.webnovel/idea_bank.json"`
...
按需加载：`cat "${SKILL_ROOT}/../../references/reading-power-taxonomy.md" "${SKILL_ROOT}/references/outlining/chapter-planning.md"`

每章必须包含：目标、阻力、代价、时间锚点、章内时间跨度、与上章时间差、倒计时状态、爽点、Strand、反派层级、视角/主角、关键实体、本章变化、章末未闭合问题、钩子，以及结构化节点——`CBN`（章节起点）、`CPNs`（推进节点 2-4个）、`CEN`（章节终点）、`必须覆盖节点`（≤4）、`本章禁区`（≤5）。

节点格式：`主体 | 动作/变化 | 对象/结果`。详细见 `references/outlining/structured-nodes.md`。相邻章节 CEN→CBN 逻辑承接。
