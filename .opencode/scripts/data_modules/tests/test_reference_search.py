...
SCRIPT = str(Path(__file__).resolve().parents[2] / "reference_search.py")
CSV_DIR = str(Path(__file__).resolve().parents[3] / "references" / "csv")
...
class TestGenreCanonical:
    def _ensure_scripts_path(self):
        """Add scripts directory to sys.path for reference_search imports."""
...
class TestEndToEndSmoke:
    ...
    def test_validate_csv_zero_errors(self):
        validate_script = str(Path(__file__).resolve().parents[2] / "validate_csv.py")
        result = subprocess.run(
