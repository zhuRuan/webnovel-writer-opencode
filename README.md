...
| **Dashboard Overview API** | 看板 API 端点，获取项目概览数据 |
...
```bash
# 1. 下载安装脚本
curl -sL https://raw.githubusercontent.com/lujih/webnovel-writer-opencode/master/install.py -o install.py

# 2. 运行安装（Linux / macOS / Windows）
python install.py
```
...
- 更新 Python 核心脚本和依赖
- 不会覆盖你的项目文件（章节、大纲、设定集等）和 .env 配置
