...
| **Graph-RAG + 持久化** | 三层子图架构，支持 SQLite 持久化，避免重启时全量重建 |
| **Code + LLM 分层审查** | Code Checkers 优先阻断严重问题 → LLM Agents 执行语义审查 |
| **DebtTracker 债务追踪** | 伏笔创建 `[伏笔:xxx]` → 偿还 `[回收:xxx]` → 硬约束阻塞 |
| **债务感知上下文预算** | 活跃债务 > 2 时自动分配 15% Token 给伏笔列表 |
| **多维度质量检查** | 设定一致性、连贯性、OOC、爽点、节奏、追读力 |
| **条件评估器** | 评估章节触发的世界规则，判断是否符合预设 |
| **题材预设通用化** | xianxia/urban/scifi 战力体系一键切换，支持自定义配置 |
| **自定义词典** | 改进中文分词质量，支持网文专有名词，自动增量重建 |
...
│   ├── agents/           # Agent 定义（context-agent, data-agent, 审查器）
...
│   │   │   ├── config_presets.py      # 世界观预设
│   │   │   ├── exceptions.py          # 统一异常体系
│   │   │   ├── rag_adapter.py        # RAG + 动态词典
│   │   │   ├── rag_backend.py        # RAG后端抽象
│   │   │   ├── temporal_graph.py     # Graph-RAG + 持久化
│   │   │   ├── context_manager.py    # 自适应上下文 + 内存缓存
...
3. 章节写作 (/webnovel-write)
   ├─→ context-agent 搜集上下文
   ├─→ 分层审查 (Code Checkers → LLM Agents)
   │   ├─ Code: world-consistency + DebtTracker
   │   └─ LLM: consistency/continuity/ooc/high-point/pacing/reader-pull
   ├─→ 债务感知上下文预算（债务 >2 → +15% foreshadow）
   ├─→ 撰写章节正文
   └─→ data-agent 更新索引 + Graph-RAG 持久化

   **批量写作** (/webnovel-write-batch)
   ├─→ 连续撰写多章节
   ├─→ 每章独立审查（可配置 minimal/standard/full）
   ├─→ 断点自动保存
   └─→ 完成后汇总报告

4. 查询设定 (/webnovel-query)
   └─→ RAG + Graph-RAG 检索相关上下文
