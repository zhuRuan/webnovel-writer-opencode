...
在项目目录（`D:\novels` 或 `~/novels`）下找到 `.env.example` 文件，复制一份命名为 `.env`，填入你的 API Key：
...
用任意文本编辑器打开 `.env`，填入你的 Embedding API Key（必填）：

```
EMBED_API_KEY=你的API_Key填在这里
```

Rerank 的 Key 是可选的，不填也能用。
...
AI 会开始和你对话，逐步收集创作信息。整个过程大约 5-10 分钟。
...
初始化完成后，需要把"总纲"细化为可执行的章纲。
> **注意**：如果你跳过了初始化阶段（比如用别人分享的项目模板），请先在 OpenCode 中切换到书项目目录，再执行以下操作。
...
如果仍然感觉"失忆"，运行一次 `update-state` 刷新状态：
```
