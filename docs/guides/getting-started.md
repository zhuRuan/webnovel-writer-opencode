...
├──────────────────────────────────────────────────────┤
│  [1] [Y] Dashboard — 管理面板  fastapi/uvicorn ~15MB  │
│  [2] [Y] 导出 MD/EPUB/HTML/DOCX  mistune/docx ~8MB   │
│  [3] [N] 发布 — 平台自动上传  playwright ~150MB       │
...
[2/3] 下载最新版本
  正在下载...
[3/3] 安装依赖
  正在安装 .opencode/scripts/requirements.txt
  （pip 进度条会实时显示）
```
...
```bash
python .opencode/scripts/webnovel.py status --project-root . -- --focus all
```

会显示已完成章节数、总字数、活跃伏笔数等。
...
当有新版本时，在 OpenCode 中切换到工作区目录，运行：
...
更多文风定制方式见 [文风自定义指南](#)（或查看 GitHub Issue #12 的回复）。
...
3. 运行索引重建：
   ```
   python .opencode/scripts/webnovel.py --project-root "./你的书名" index process-chapter --chapter 1
   ```
