...
```json
{
  ...
  "entry_points": {
    ...
    "hooks": [
      {
        "id": "my-hook",
        "class": "hooks.my_hook.MyHook",
        "trigger_on": ["after_review", "before_polish"],
        "description": "审查后自动修复敏感词",
        "priority": 10
      }
    ]
  },
...
| `entry_points` | object | 是 | 注册扩展点（Agent、Skill、Checker、Publisher、Hook）。 |
| `entry_points.agents` | array | 否 | Agent 扩展点列表。 |
| `entry_points.skills` | array | 否 | Skill 扩展点列表。 |
| `entry_points.checkers` | array | 否 | Checker 扩展点列表。 |
| `entry_points.publishers` | array | 否 | Publisher 扩展点列表。 |
| `entry_points.templates` | array | 否 | 题材模板列表。 |
| `entry_points.hooks` | array | 否 | 工作流钩子列表（见下方详细说明）。 |
| `permissions` | array | 否 | 插件需要的权限列表（如 `read:chapters`），用于权限沙箱。 |

### 3.4 钩子注册格式（entry_points.hooks）

钩子允许插件在写作/审查流程的关键节点注入逻辑。

| 字段 | 类型 | 说明 |
|------|------|------|
| `id` | string | 钩子唯一标识 |
| `class` | string | 钩子类路径（如 `hooks.my_hook.MyHook`） |
| `trigger_on` | array | 触发点列表，可选值：`before_write`、`after_context`、`before_draft`、`after_draft`、`before_review`、`after_review`、`before_polish`、`after_polish`、`before_data_write`、`after_data_write` |
| `description` | string | 钩子功能描述 |
| `priority` | int | 执行优先级，数字越小越先执行（默认 100） |

示例：
```json
"hooks": [
  {
    "id": "auto-fix-sensitive",
    "class": "hooks.sensitive_fix.SensitiveWordFixHook",
    "trigger_on": ["after_review", "before_polish"],
    "description": "审查后自动修复敏感词",
    "priority": 10
  }
...
class MyHook(BaseHook):
    ...
    async def trigger(self, context: dict) -> dict:
        """
        执行钩子逻辑
        ...
        Args:
            context: 包含当前流程上下文
                - chapter: 章节号
                - chapter_content: 章节正文（可选）
                - review_result: 审查结果（仅 after_review 可用）
                - issues: 问题列表（仅 after_review 可用）
    ...
    def get_hook_points(self) -> list:
        """返回钩子触发点列表"""
        return ["after_review", "before_polish"]

    def validate_context(self, context: dict) -> bool:
        """验证输入上下文"""
        return "chapter" in context
```
...
| 钩子点 | 输入上下文字段 |
|--------|----------------|
| `before_write` | `chapter`, `project_root` |
| `after_context` | `chapter`, `project_root`, `context_data` |
| `before_draft` | `chapter`, `project_root`, `task`, `contract` |
| `after_draft` | `chapter`, `project_root`, `chapter_content` |
| `before_review` | `chapter`, `project_root`, `chapter_content` |
| `after_review` | `chapter`, `project_root`, `chapter_content`, `review_result`, `issues` |
| `before_polish` | `chapter`, `project_root`, `chapter_content`, `review_result`, `issues` |
| `after_polish` | `chapter`, `project_root`, `chapter_content`, `changes` |
| `before_data_write` | `chapter`, `project_root`, `chapter_content`, `review_result` |
| `after_data_write` | `chapter`, `project_root`, `chapter_content`, `state_updated`, `index_updated` |
...
```bash
# 列出所有已注册的钩子及其触发点
python .opencode/scripts/webnovel.py hook list

# 执行特定钩子点（文件输入/输出）
python .opencode/scripts/webnovel.py hook run --point after_review --input '@review.json' --output '@output.json'

# 调试模式（失败时终止，便于调试）
python .opencode/scripts/webnovel.py hook run --point after_review --input '@review.json' --strict
```
...
| 钩子点 | 说明 | 典型用途 |
|--------|------|----------|
| `before_write` | 写作前 | 预加载数据、初始化上下文 |
| `after_context` | 上下文生成后 | 增强检索结果 |
| `before_draft` | 起草前 | 准备写作提示词 |
| `after_draft` | 起草后 | 预检查草稿 |
| `before_review` | 审查前 | 预处理待审内容 |
| `after_review` | 审查后 | 自动修复问题 |
| `before_polish` | 润色前 | 准备润色参数 |
| `after_polish` | 润色后 | 验证润色结果 |
| `before_data_write` | 数据回写前 | 准备索引数据 |
| `after_data_write` | 数据回写后 | 触发后续任务 |
